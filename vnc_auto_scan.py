...
def get_fqdn(ip):
    try:
        ...
        return fqdn if fqdn != str(ip) else None
    except:
        return None
...
def create_vnc_file(ip, hostname, fullname, username):
...
fullname={fullname if fullname else 'Unknown'}
...
def scan_ip(ip):
    if ping(ip) and is_port_open(ip):
        ...
        fullname = get_fqdn(ip)
        username = get_username(ip)
        create_vnc_file(ip, hostname, fullname, username)
        return (str(ip), hostname if hostname else "Unknown", fullname if fullname else "Unknown", username if username else "Unknown")
    return None
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=50) as executor:
        ...
        for future in as_completed(futures):
            ...
            if result:
                ip, hostname, fullname, username = result
                print(f"{ip} -> Hostname: {hostname}, Fullname: {fullname}, User: {username}")
