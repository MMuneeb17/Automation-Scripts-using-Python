...
def save_to_excel(results, local_ip):
    """Save ping results to Excel file on Desktop - appends if file exists"""
    # Get Desktop path
    ...
    if excel_path.exists():
        ...
        # Add separator row
        last_row += 2
        ws.cell(last_row, 1, f"--- Test Run: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ---")
        ws.cell(last_row, 1).font = Font(bold=True, italic=True, size=10)
        ...
        ws['A2'] = f"Local IP Address: {local_ip}"
        ws['A3'] = f"First Test: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
