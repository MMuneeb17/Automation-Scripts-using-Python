...
def export_to_excel(results, filename="Enhanced Network Scanner with Critical IP Testing.xlsx"):
    """
        ...
        if critical_connectivity:
            for target_ip, label in CRITICAL_IPS:
                ...
                if isinstance(connectivity_data, dict):
                    status = connectivity_data.get('status', 'N/A')
    ...
    try:
        ...
        if active_hosts:
            ...
            for ip, status, hostname, computer_name, username, critical_connectivity in active_hosts:
                ...
                # Show detailed connectivity breakdown with response times
                if critical_connectivity:
                    for target, conn_data in critical_connectivity.items():
                        if isinstance(conn_data, dict):
                            status = conn_data.get('status', 'Unknown')
                            response_time = conn_data.get('response_time', 'N/A')
                            status_symbol = "✓" if "Reachable" in status else "✗"

                            if response_time != 'N/A':
                                print(f"      {status_symbol} {target}: {status} - Response Time: {response_time}")
                            else:
                                print(f"      {status_symbol} {target}: {status}")
                        else:
                            status_symbol = "✓" if "Reachable" in str(conn_data) else "✗"
                            print(f"      {status_symbol} {target}: {conn_data}")
